...
def run_pipeline(wav_path, transcriber, summarizer):
    ...
    # summary
    # the rendered transcript has one utterance per line, which is what the LLM chunker splits on
    summary_md = summarizer.summarize(transcript_md)
...
# Whisper and the LLM stay loaded on the Hailo for the whole process, only the CPU diarizer is loaded per job.
# gc is Python's built-in garbage collector module
async def pipeline_worker(app):
...
async def lifespan(app):
    ...
    with VDevice() as vdevice:
        app.state.transcriber = Transcribe(vdevice)
        app.state.summarizer = Summarize(vdevice)
        ...
        try:
            ...
            with contextlib.suppress(asyncio.CancelledError):
                await worker
            # the models must be released before the VDevice closes
            app.state.transcriber.close()
            app.state.summarizer.close()
